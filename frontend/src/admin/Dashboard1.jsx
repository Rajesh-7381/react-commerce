  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // console.log("Count data received:", response.data);
        if (response.data && response.data.count !== undefined) {  //checks if the response.data object exists and if it contains a count property. This is important to avoid errors in cases where the network request might succeed but the expected data isn't returned.
          setCount(response.data.count); // Access using 'count' directly, no bracket notation needed
